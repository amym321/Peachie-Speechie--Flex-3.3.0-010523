...
var PriceUI = /*#__PURE__*/function () {
  function PriceUI(el) {
    dist_index_es_classCallCheck(this, PriceUI);

    this._el = el;
  }

  dist_index_es_createClass(PriceUI, [{
    key: "load",
    value: function load(product, options) {
      var _variant$formatter$ha = dist_index_es_objectSpread2({
        variant: false,
...
var PriceUIBadge = /*#__PURE__*/(/* unused pure expression or super */ null && (function () {
  function PriceUIBadge(el) {
    dist_index_es_classCallCheck(this, PriceUIBadge);

    this._el = el;
  }

  dist_index_es_createClass(PriceUIBadge, [{
    key: "load",
    value: function load(product, options) {
      var _variant$style$format = dist_index_es_objectSpread2({
        variant: false,
        style: 'percent',
        formatter: function formatter(price) {
          return price;
...
function shopify_asyncview_dist_index_es_defineProperty(obj, key, value) {
  if (key in obj) {
    Object.defineProperty(obj, key, {
      value: value,
      enumerable: true,
...
function shopify_asyncview_dist_index_es_ownKeys(object, enumerableOnly) {
  var keys = Object.keys(object);
...
function shopify_asyncview_dist_index_es_objectSpread2(target) {
  for (var i = 1; i < arguments.length; i++) {
    var source = arguments[i] != null ? arguments[i] : {};

    if (i % 2) {
      shopify_asyncview_dist_index_es_ownKeys(Object(source), true).forEach(function (key) {
        shopify_asyncview_dist_index_es_defineProperty(target, key, source[key]);
      });
    } else if (Object.getOwnPropertyDescriptors) {
      Object.defineProperties(target, Object.getOwnPropertyDescriptors(source));
    } else {
      shopify_asyncview_dist_index_es_ownKeys(Object(source)).forEach(function (key) {
        Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key));
      });
    }
  }
...
var index_es_AsyncView = /*#__PURE__*/function () {
  ...
  shopify_asyncview_dist_index_es_createClass(AsyncView, null, [{
    ...
    value: function load(url) {
      ...
      var cachebustingParams = shopify_asyncview_dist_index_es_objectSpread2({}, queryParams, {
        _: new Date().getTime()
      });
...
class Product {
  ...
  constructor($section) {
    ...
    if ($('.masonry--true').length > 0) {
      window.PXUTheme.applyMasonry('.thumbnail');
                console.log('.masonry--true.length > 0'); //doesn't hit
  ...
  enableSlideshow(selector, settings) {
    ...
    const $thumbnails = $thumbnailProductGallery.find('.product-gallery__thumbnail');
            ...
            var thumbObj = JSON.stringify($thumbnails, null, 2);
            console.log('log 14) thumbObj = '+thumbObj); // 6 items for 6 thumbs
    ...
    if (($sliderArrows || $thumbnails) && window.isScreenSizeLarge()) {
      ...
      $thumbnails.on('click', event => {
        ...
        $productGallery.flickity('select', index);
                    ...
                    console.log('log 15) $thumbnails.on click'); // hits when img clicked but NOT when swatch clicked
                    // $thumbnailProductGallery.find('.is-nav-selected').removeClass('is-nav-selected');
                    // const $selected = $thumbnails.eq(flkty.selectedIndex).addClass('is-nav-selected');


        $productGallery.on('settle.flickity', () => {
          console.log('log 16) $productGallery.on settle.flickity');// hits when img clicked but NOT when swatch clicked
          ...
          const pId = ($productGallery).data('product-id');
                    ...
                    // //console.log('log 17) pId = '+pId);// same id irrespective of img clicked
                    // console.log('log 18) $selectedSlide = '+ $selectedSlide);// object Object
                    // var selectedSlideO = $productGallery.find('.product-gallery__image.is-selected');
                    // // var selectedSlideObj = JSON.stringify(selectedSlideO);// error Converting circular structure to JSON
                    // // console.log('log 20) selectedSlideObj = '+ selectedSlideObj);
                    // for(let key in selectedSlideO) {
                    //   console.log('log 21)');
                    //   console.log(key + ":", selectedSlideO[key]); 
                    // } //got 156 returns. 1st related to main img. thus $selectedSlide is in $productGallery, main img
    ...
    if (thumbnailsEnabled === true && thumbnailsSliderEnabled === true && $slides.length > 1) {
      ...
      // slider wraparound/offset only when more than 3 thumbs - am
      if ($thumbnails.length <= 3){
        var flickWrapAround = false;
        var flickFreeScroll = false;
      } else {
        var flickWrapAround = true;
        var flickFreeScroll = true;
      }
      ...
      if (window.isScreenSizeLarge()) {
        if (thumbnailsPosition === 'right-thumbnails' || thumbnailsPosition === 'left-thumbnails') {
          ...
          $thumbnailProductGallery.flickity({
            ...
            arrowShape: window.arrowShape,
            wrapAround: flickWrapAround,
            freeScroll: flickFreeScroll,
          });
  ...
  relatedProducts() {
    $('.js-related-products-slider .products-slider').each((_, slider) => {
      ...
      const slideData = {
        ...
        products_available: $relatedSlider.data('products-available'), // 10
      ...
      if (
        ...
        $relatedSlider.flickity({
          ...
          resize: true, // cell size, snaps slider into place
          //resize: false,
